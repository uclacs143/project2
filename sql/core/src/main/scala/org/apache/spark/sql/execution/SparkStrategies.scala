...
private[sql] abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object HashAggregation extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case PartialAggregation(
      namedGroupingAttributes,  // Grouping attributes for the final aggregation.
      rewrittenAggregateExpressions,  // Aggregates for the final aggregation
      groupingExpressions,  // Grouping expressions for the partial aggregation
      partialComputation, // Partial aggregate expressions
      child)  if !canBeCodeGened(
        allAggregates(partialComputation) ++ allAggregates(rewrittenAggregateExpressions)) &&
          (rewrittenAggregateExpressions.length==1) =>
      ...
      child) if !canBeCodeGened(
        allAggregates(partialComputation) ++
          allAggregates(rewrittenAggregateExpressions)) && (rewrittenAggregateExpressions.length>1) =>
        execution.Aggregate(
          partial = false,
          namedGroupingAttributes,
          rewrittenAggregateExpressions,
