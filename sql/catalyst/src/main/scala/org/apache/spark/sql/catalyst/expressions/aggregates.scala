...
case class AverageFunction(expr: Expression, base: AggregateExpression)
  ...
  private val zero: Cast = Cast(Literal(0), calcType)
  ...
  /** This method computes the actual average as sum / count. The tricky part here is doing division while maintaining
    ...
    */
  override def eval(input: Row): Any = {
    if (count.eval(null) == zero) {
      null
    } else {
      expr.dataType match {
        case calcType =>
          Divide(Cast(sum, DoubleType), Cast(count, DoubleType)).eval(null)
  ...
  override def update(input: Row): Unit = {
    ...
    if (evaluatedExpr != null) {
      count.update(countFunction(), null)
